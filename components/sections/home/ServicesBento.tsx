          ...
          {items.map((service, i) => {
            ...
            return (
              <TiltCard key={service.title} className={cn(spans[i])}>
                <div
                  className={cn(
                    "glass relative flex h-full flex-col justify-between overflow-hidden rounded-2xl p-4 shadow-[0_1px_0_0_rgba(255,255,255,0.6)_inset] transition-all duration-300 group-hover:border-[var(--color-accent)]/30 group-hover:shadow-[0_0_40px_-8px_var(--color-accent)] sm:p-5 md:p-6",
                ...
                >
                  <div className="flex items-start justify-between">
                    <div
                      className={cn(
                        "flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--color-accent)] text-white sm:h-10 sm:w-10",
